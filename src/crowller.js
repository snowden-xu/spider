...
var Crawler = /** @class */ (function () {
    function Crawler() {
        this.token = "token";
        this.url = "http://sc-light.com/?productlist/";
        this.initSpiderProcess();
    }
    Crawler.prototype.getCourseInfo = function (html) {
        var $ = cheerio.load(html);
        var courseItems = $(".project-content-right ul li");
        var courseInfos = [];
        courseItems.map(function (index, element) {
            var src = $(element).find("img").attr("src");
            var title = $(element).find(".project-title").eq(0).text();
            courseInfos.push({ src: src, title: title });
        });
        return {
    ...
    Crawler.prototype.generateJsonContent = function (courseInfo) {
